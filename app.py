...
def extrair_dados_universal(texto_copiado, tipo_selecionado):
    ...
    # ESTRATÉGIA DE CORTE: QUEBRA POR NOME DE ADMIN (Funciona para ambos os sites)
    admins_regex = r'(?i)(bradesco|santander|itaú|itau|porto|caixa|banco do brasil|bb|rodobens|embracon|ancora|âncora|mycon|sicredi|sicoob|mapfre|hs|yamaha|zema|bancorbrás|bancorbras|servopa|disal|volkswagen|chevrolet)'
    
    # Separa mantendo o delimitador (o nome do banco)
    partes = re.split(f'({admins_regex})', texto_limpo)
    
    # Reconstrói os blocos (Nome Admin + Dados Seguintes)
    blocos = []
    # O split gera [lixo, Admin1, Dados1, Admin2, Dados2...]
    for i in range(1, len(partes), 2):
        if i+1 < len(partes):
            # Junta o nome da admin (partes[i]) com o conteúdo (partes[i+1])
            blocos.append(partes[i] + " " + partes[i+1])

    # Se a quebra por admin falhou (texto muito curto ou formato estranho), tenta quebra visual
    if len(blocos) == 0:
        blocos = re.split(r'\n\s*\n', texto_limpo)
    ...
    for bloco in blocos:
        ...
        # Identifica Admin (Reforço)
        admins_lista = ['BRADESCO', 'SANTANDER', 'ITAÚ', 'ITAU', 'PORTO', 'CAIXA', 'BANCO DO BRASIL', 'BB', 'RODOBENS', 'EMBRACON', 'ANCORA', 'ÂNCORA', 'MYCON', 'SICREDI', 'SICOOB', 'MAPFRE', 'HS', 'YAMAHA', 'ZEMA', 'BANCORBRÁS', 'BANCORBRAS', 'SERVOPA', 'DISAL', 'VOLKSWAGEN', 'CHEVROLET']
        admin_encontrada = "OUTROS"
        for adm in admins_lista:
            if adm.lower() in bloco_lower:
                admin_encontrada = adm.upper()
                break
        
        # Permite OUTROS apenas se tiver cara de cota (tem R$)
        if admin_encontrada == "OUTROS" and "r$" not in bloco_lower: continue

        # --- ESTRATÉGIA HÍBRIDA DE VALORES ---
        credito = 0.0
        entrada = 0.0
        
        # 1. TENTA POR RÓTULO (Site TopContempladas)
        ...
        # 2. TENTA POR POSIÇÃO/ORDEM DE GRANDEZA (Site Piffer/WhatsApp)
        # Se falhou nos rótulos, pega todos os valores e ordena
        if credito == 0 or entrada == 0:
            valores = re.findall(r'R\$\s?([\d\.,]+)', bloco)
            vals_float = sorted([limpar_moeda(v) for v in valores], reverse=True)
            
            # Assume que o maior valor é Crédito e o segundo é Entrada (padrão 99% das vezes)
            if len(vals_float) >= 1 and credito == 0: credito = vals_float[0]
            if len(vals_float) >= 2 and entrada == 0: entrada = vals_float[1]

        # --- ESTRATÉGIA HÍBRIDA DE PARCELAS ---
        saldo_devedor = 0.0
        parcela_teto = 0.0
        
        # Regex Universal: Pega "100x R$ 1.000", "100 x 1000", "100 parcelas de R$ 1000"
        # O padrão `(\d+)\s*[xX]\s*` pega o "x".
        # O padrão `de\s*R\$` pega o "de R$".
        regex_hibrida = r'(\d+)\s*(?:[xX]|vezes|parcelas.*?de)\s*R?\$\s?([\d\.,]+)'
        todas_parcelas = re.findall(regex_hibrida, bloco, re.IGNORECASE)
        
        # Se não achou com 'x', tenta o inverso: "R$ 1.000 em 100 meses"
        if not todas_parcelas:
             todas_parcelas_inv = re.findall(r'R?\$\s?([\d\.,]+)\s*(?:em|x|durante)\s*(\d+)', bloco, re.IGNORECASE)
             # Inverte para ficar (Prazo, Valor)
             if todas_parcelas_inv:
                 todas_parcelas = [(p[1], p[0]) for p in todas_parcelas_inv]

        for pz_str, vlr_str in todas_parcelas:
            pz = int(pz_str)
            vlr = limpar_moeda(vlr_str)
            
            # Filtro de erro: Ignora "parcelas" que parecem anos (ex: 2025) ou valores irrisórios
            if pz > 360 or vlr < 50: continue 
            
            saldo_devedor += (pz * vlr)
            # Pega a maior parcela encontrada como referência de fluxo mensal
            if pz > 1 and vlr > parcela_teto: parcela_teto = vlr
            elif len(todas_parcelas) == 1: parcela_teto = vlr

        if credito > 0 and entrada > 0:
            # Fallback para Saldo Devedor se não achou parcelas (comum em sites resumidos)
            if saldo_devedor == 0: 
                # Estima um saldo para não quebrar o cálculo, mas prioriza o real se existir
                saldo_devedor = max(0, (credito * 1.3) - entrada) 

            custo_total = entrada + saldo_devedor
            
            # Só aceita se for uma cota válida (Crédito > 5k)
            if credito > 5000: 
                lista_cotas.append({
                    'ID': id_cota, 
                    'Admin': admin_encontrada, 
                    'Tipo': tipo_selecionado, # OBEDECE AO USUÁRIO
